...
String AllMenus::pickALineInMenus(int menuIndex,int menuRowIndex) {
  String findstr;
  char cmenuIndex = 48+menuIndex;//48是字符'0'的十进制ASCII码
  char cmenuRowIndex = 48+menuRowIndex;
  findstr = menusAndTheirsContent.substring(menusAndTheirsContent.indexOf(cmenuIndex),menusAndTheirsContent.indexOf(cmenuIndex+1));// int String::indexOf(char Index),返回字符串中从前往后查找到的第一个Index的位置；String String::substring(int from,int to=String.lenth()+1)，返回一个字符串中从位置from至to的字串。一个String的第一个字符的位置是0.
...
void setup() {
  ...
  allmenus->menusAndTheirsContent = "1 aaaaa bbbb a a a 2 bbbbb b b b b " ;
  allmenus->pickALineInMenus(2,1);
